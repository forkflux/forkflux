...
- Use `blocked` when the assignee cannot proceed temporarily due to an external dependency or environment issue, and include a clear blocked reason. API and CLI clients can record a cleared blocker as `unblocked` with an `unblock_reason` before resuming `in_progress`; the current MCP status tool resumes blocked jobs directly as `in_progress`.

...
2. Validate that the target status is one of the MCP tool's supported states: `blocked`, `in_progress`, `completed`, `failed`, or `cancelled`. Use `in_progress` only to resume a previously blocked or failed job; do not use it for normal claiming.

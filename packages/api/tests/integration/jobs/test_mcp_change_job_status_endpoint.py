...
async def test_change_job_status_returns_422_when_resuming_directly_from_blocked(
    client: AsyncClient,
    db_session: AsyncSession,
) -> None:
    ...
    assert persisted_job is not None
    assert persisted_job.status == JobStatusEnum.BLOCKED
    assert persisted_job.blocked_reason == "waiting on upstream dependency"
    assert persisted_job.blocked_at == old_timestamp
...
async def test_change_job_status_returns_422_when_unblock_reason_missing(
    client: AsyncClient,
    db_session: AsyncSession,
) -> None:
    raw_token = "valid-change-status-unblock-missing-reason-token"
    assignee_id, assignee_role_id = await _create_authenticated_agent(
        db_session,
        raw_token=raw_token,
        role_key="change-status-unblock-missing-reason-assignee-role",
        role_label="Change status unblock missing reason assignee role",
        agent_label="change-status-unblock-missing-reason-assignee-agent",
    )

    source_agent = await AgentIdentityFactory.create(
        db_session,
        agent_label="change-status-unblock-missing-reason-source-agent",
    )
    ...
    response = await client.post(
        f"/api/v1/mcp/jobs/{job.id}/status",
        json={"status": JobStatusEnum.UNBLOCKED.value},
        headers={"Authorization": f"Bearer {raw_token}"},
    )

...
describe('updateRole', () => {
  ...
  it('sends PATCH to the correct endpoint with the right payload', async () => {
    const mockRole = {
      id: 10,
      role_key: 'qa',
      role_label: 'QA Tester',
    ...
    const result = await apiDataSource.updateRole(10, 'qa', 'QA Tester')

    expect(fetchMock).toHaveBeenCalledTimes(1)
    const [url, options] = fetchMock.mock.calls[0] as [string, RequestInit]
    expect(url).toBe(`${API_BASE}/ui/agents/roles/10`)
  ...
  it('throws a conflict error on 422 with target_role.conflict code', async () => {
    ...
    await expect(
      apiDataSource.updateRole(1, 'backend', 'Backend Engineer'),
    ).rejects.toThrow('A role with the key "backend" already exists.')
  })

  it('throws a not-found error on 422 with target_role.not_found code', async () => {
    fetchMock.mockResolvedValue({
      ok: false,
      status: 422,
      statusText: 'Unprocessable Entity',
    ...
    await expect(
      apiDataSource.updateRole(999, 'qa', 'QA Tester'),
  ...
  it('throws the backend message on non-conflict 422 (validation error)', async () => {
    ...
    await expect(
      apiDataSource.updateRole(1, 'short', 'QA Tester'),
    ).rejects.toThrow('String should have at most 255 characters')
  })
  ...
  it('throws a fallback message on 422 with unparseable body', async () => {
    ...
    await expect(
      apiDataSource.updateRole(1, 'qa', 'QA Tester'),
    ).rejects.toThrow('Invalid input. Please check your values.')
  })
  ...
  it('throws client-side error when role_key exceeds 255 chars', async () => {
    ...
    await expect(
      apiDataSource.updateRole(1, longKey, 'QA Tester'),
    ).rejects.toThrow('Role key must be at most 255 characters.')
  ...
  it('throws client-side error when role_label exceeds 255 chars', async () => {
    ...
    await expect(
      apiDataSource.updateRole(1, 'qa', longLabel),
    ).rejects.toThrow('Role label must be at most 255 characters.')
...
describe('deleteRole', () => {
  ...
  it('sends DELETE to the correct endpoint and resolves void on 204', async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      status: 204,
    ...
    await apiDataSource.deleteRole(10)
  ...
  it('throws a not-found error on 422 with target_role.not_found code', async () => {
    ...
    await expect(
      apiDataSource.deleteRole(999),
    ).rejects.toThrow('This role no longer exists. Please refresh and try again.')
  })

  it('throws the backend message on non-not-found 422', async () => {
    fetchMock.mockResolvedValue({
      ok: false,
      status: 422,
      statusText: 'Unprocessable Entity',
